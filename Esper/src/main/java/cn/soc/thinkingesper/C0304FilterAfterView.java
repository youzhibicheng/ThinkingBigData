...
// 在进入view之前过滤
class C0304AppleListener implements UpdateListener {
...
public class C0304FilterAfterView {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		String epl = "select id, price, amount from " + product + ".win:length_batch(3) where amount>200";

		EPStatement state = admin.createEPL(epl);
		state.addListener(new C0304AppleListener());

		EPRuntime runtime = epService.getEPRuntime();

		// 1,2,3 都进入 view, 出发，但是因为2不满足过滤条件，所以并没有进入newEvents[]数组,只打印1,,3
		...
		// 4,5,6 都进入 view, 出发，但是因为5不满足过滤条件，所以并没有进入newEvents[]数组,只打印4,6
