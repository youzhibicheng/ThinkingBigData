...
class MergeEvent implements Serializable {
	...
	public String toString() {
		return "mergeId=" + mergeId + ", mergeStr=" + mergeStr + ", mergeSize=" + mergeSize + ", deleteFlag="
				+ deleteFlag;
	}
}
...
class SelectLogWindowlistener implements UpdateListener {
	public void update(EventBean[] newEvents, EventBean[] oldEvents) {
		if (newEvents != null) {
			System.out.println("Trigger SelectLogWindowlistener:");
			for (int i = 0; i < newEvents.length; i++) {
				System.out.println(newEvents[i].getUnderlying());
			}
		}
...
class SelectMergeWindowlistener implements UpdateListener {
	public void update(EventBean[] newEvents, EventBean[] oldEvents) {
		if (newEvents != null) {
			System.out.println("Trigger SelectMergeWindowlistener:");
			for (int i = 0; i < newEvents.length; i++) {
				System.out.println(newEvents[i].getUnderlying());
			}
		}
...
public class C1206OnMergeWindowTest {
